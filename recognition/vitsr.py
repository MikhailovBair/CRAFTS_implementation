...
## 카카오에선 컬럼 와이즈로 자른다고 한다.
def vitstr_columnwise_crop(pretrained=False, **kwargs) :
    
    model = ViTSTR(
        patch_size = (32,5), img_size = (32,140), embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True, in_chans = 1, **kwargs
    )
    return model
