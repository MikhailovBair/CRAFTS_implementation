...
from recognition.vitsr import create_vitstr, ViTSTR
...
class TRBA(nn.Module):
    ...
    def __init__(self, opt):
        ...
        if opt.Transformer:
            self.vitstr = ViTSTR(img_size = (opt.imgH, opt.imgW),
                                 in_chans = opt.input_channel,
        ...
        elif opt.FeatureExtraction == 'SimpleResNet' :
            self.FeatureExtraction = SimpleResNet_FeatureExtractor(opt.input_channel)
        else:
    ...
    def forward(self, input, text, is_train=True):
        ...
        if self.stages['ViTSTR']:
            prediction = self.vitstr(input, seqlen=17)
            return prediction    
