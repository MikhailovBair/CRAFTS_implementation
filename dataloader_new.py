...
from recognition.utils import CTCLabelConverter, CTCLabelConverterForBaiduWarpctc, AttnLabelConverter, \
    TokenLabelConverter, Averager
...
class Dataset(data.Dataset):
    def __init__(self,
        ...
        if type(self.data_folder) == list:
            ...
            for folder in self.data_folder:
                imgfiles = glob(os.path.join(folder, 'Train_images', '*.jpg')) + glob(
                    os.path.join(folder, 'Train_images', '*.png'))
                gts = [imgfile.replace('Train_images', 'Train_gts').replace('.jpg', '.txt').replace('.png', '.txt') for
                       imgfile in imgfiles]
                self.images_path.extend(imgfiles)
        else:
            imgfiles = glob(os.path.join(self.data_folder, 'Train_images', '*.jpg')) + glob(
                os.path.join(self.data_folder, 'Train_images', '*.png'))
            ...
            if h > w * 1.5 and len(word) != 1:
                ...
                if rot_angle == 90 or rot_angle == None or rot_angle == 0:
                    M = cv2.getPerspectiveTransform(np.float32(box),
                                                    np.float32(
                                                        np.array([[0, height], [0, 0], [width, 0], [width, height]])))
                elif rot_angle == 270:
                    M = cv2.getPerspectiveTransform(np.float32(box),
                                                    np.float32(
    ...
    def load_image_gt_and_confidencemask(self, index):
        ...
        gt_path = imagename.replace('Train_images', 'Train_gts').replace('.jpg', '.txt').replace('.png',
                                                                                                 '.txt')  # os.path.join(self.gt_folder, "%s.txt" % os.path.splitext(imagename)[0])
    ...
    def pull_item(self, index):
        ...
        image_batches = []
        if len(character_bboxes) > 0:
            region_scores = self.gaussianTransformer.generate_region(region_scores.shape,
                                                                     character_bboxes)  # 0-255 range
            affinity_scores, affinity_bboxes = self.gaussianTransformer.generate_adjust_affinity(region_scores.shape,
                                                                                                 character_bboxes,
                                                                                                 words)

            try:
                orientation_x, orientation_y = create_orientation(affinity_bboxes, character_bboxes, region_scores,
                                                                  words)
            except:
                print(index)
                raise ValueError
        ...
        cropped_word_bboxes, cropped_words = np.zeros((self.str_batch_size, 4, 2), np.float32), [
            self.PAD] * self.str_batch_size
        ...
        if self.str_cfg.ViTSTR:
            ...
            encoded_cropped_words, words_length = self.converter.encode(cropped_words, batch_max_length=int(
                self.str_cfg.batch_max_length))
