...
if __name__ == '__main__':
    ...
    print('model input parameters', str_cfg.imgH, str_cfg.imgW, str_cfg.num_fiducial, str_cfg.input_channel,
          str_cfg.output_channel,
          str_cfg.hidden_size, str_cfg.num_class, str_cfg.batch_max_length, str_cfg.Transformation,
          str_cfg.FeatureExtraction,
    ...
    for epoch in range(cfg.MAX_EPOCH):
        ...
        for index, (
        images, gh_label, gah_label, mask, ori_x, ori_y, word_bboxes_batch, words_batch, words_length_batch) in tqdm(
            ...
            STR_inputs = torch.cat([out[:, :2, :, :], feature], axis=1).permute(0, 2, 3,
                                                                                1).cpu().detach().numpy()  # receive str_input as (batch_size, input_)
            ...
            if index % 100 == 0 and index > 0:
                et = time.time()
                print('epoch {}:({}/{}) batch || training time for 1 batch {} || training loss {}'.format(epoch, index,
                                                                                                          len(data_loader),
                                                                                                          et - st,
                                                                                                          loss_value))
                loss_time = 0
                ...
                torch.save(model.state_dict(),
                           os.path.join('./saved_models/', cfg.EXP_NAME, 'CRAFTS' + repr(epoch) + '.pth'))
