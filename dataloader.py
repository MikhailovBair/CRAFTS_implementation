...
from watershed import watershed, watershed_wo_net
import re
import itertools
from file_utils import *
from mep import mep
...
from data_utils import sort_rectangle, create_orientation, crop_image_batch_by_bbox
...
class Dataset(craft_base_dataset) :
    def __init__(self, cfg, delimiter = ',',  orientation = True, target_size = 768, viz = False, debug = False, use_net = False, watershed_on = False) :
        ...
        if 'CTC' in self.str_cfg.Prediction:
            if self.str_cfg.baiduCTC:
                self.converter = CTCLabelConverterForBaiduWarpctc(character_list)
            else:
                self.converter = CTCLabelConverter(character_list)
        else:
    ...
    def load_gt(self, gt_path):
        ...
        words = []
        for line in lines:
            txt_info = line.strip().encode('utf-8').decode('utf-8-sig').split(self.delimiter)     
            if len(txt_info) == 1:
                if self.delimiter == ',' :
                    delimiter = '\t'
                else :
                    delimiter = ','
                txt_info = line.strip().encode('utf-8').decode('utf-8-sig').split(delimiter)
            
            if len(txt_info) == 9 :
                box = [int(float(txt_info[j])) for j in range(8)]
                word = txt_info[-1].replace('@@@','')
                if word == '###' or word == '' :
                    word = '[UNK]'
                word = word.replace('###', '')
                word = txt_info[-1]
                ...
                if len(txt_info )<9 :
                    ...
                    continue
                box = [int(float(txt_info[j])) for j in range(len(txt_info)-1)]
                word = txt_info[-1].replace('@@@','')
                if word == '###' or word == '' :
                    word = '[UNK]'
                word = word.replace('###', '')
    ...
    def inference_pursedo_bboxes(self, image, word_bbox, word, viz=False):
        ...
        # implement textsnake instead of watershed 
        # link : https://github.com/princewang1994/TextSnake.pytorch
...
#         return image, character_bboxes, words, confidence_mask, confidences, word_bboxes, image.shape[:2]
    ...
    def pull_item(self, index):
        ...
        ## TODO : data augmentation. 현재 코드에서는 좌표값 망가짐.
